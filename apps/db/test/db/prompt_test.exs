defmodule GifMe.DB.PromptTest do
  ...
  test "type can't be blank" do
    assert {:error, %Ecto.Changeset{} = changeset} = Prompts.create_prompt(Map.delete(@valid_attrs, :type))
    assert %{type: ["can't be blank"]} = errors_on(changeset)
  end

  test "source can't be blank" do
    assert {:error, %Ecto.Changeset{} = changeset} = Prompts.create_prompt(Map.delete(@valid_attrs, :source))
    assert %{source: ["can't be blank"]} = errors_on(changeset)
  end

  test "url can't be blank" do
    assert {:error, %Ecto.Changeset{} = changeset} = Prompts.create_prompt(Map.delete(@valid_attrs, :url))
